...
pub fn idct8x8_simd(a: &mut [f32; 64]) {
    {
        ...
        x1r = f32x8::splat(C8_1R) * f32x8::from_slice(&a[1 * 8..1 * 8 + 8])
            + f32x8::splat(C8_1I) * f32x8::from_slice(&a[7 * 8..7 * 8 + 8]);
        x1i = f32x8::splat(C8_1R) * f32x8::from_slice(&a[7 * 8..7 * 8 + 8])
            - f32x8::splat(C8_1I) * f32x8::from_slice(&a[1 * 8..1 * 8 + 8]);
        x3r = f32x8::splat(C8_3R) * f32x8::from_slice(&a[3 * 8..3 * 8 + 8])
            + f32x8::splat(C8_3I) * f32x8::from_slice(&a[5 * 8..5 * 8 + 8]);
        x3i = f32x8::splat(C8_3R) * f32x8::from_slice(&a[5 * 8..5 * 8 + 8])
            - f32x8::splat(C8_3I) * f32x8::from_slice(&a[3 * 8..3 * 8 + 8]);
        xr = x1r - x3r;
        xi = x1i + x3i;
        x1r += x3r;
        x3i -= x1i;
        x1i = f32x8::splat(W8_4R) * (xr + xi);
        x3r = f32x8::splat(W8_4R) * (xr - xi);
        xr = f32x8::splat(C8_2R) * f32x8::from_slice(&a[2 * 8..2 * 8 + 8])
            + f32x8::splat(C8_2I) * f32x8::from_slice(&a[6 * 8..6 * 8 + 8]);
        xi = f32x8::splat(C8_2R) * f32x8::from_slice(&a[6 * 8..6 * 8 + 8])
            - f32x8::splat(C8_2I) * f32x8::from_slice(&a[2 * 8..2 * 8 + 8]);
        x0r = f32x8::splat(C8_4R)
            * (f32x8::from_slice(&a[0 * 8..0 * 8 + 8]) + f32x8::from_slice(&a[4 * 8..4 * 8 + 8]));
        x0i = f32x8::splat(C8_4R)
            * (f32x8::from_slice(&a[0 * 8..0 * 8 + 8]) - f32x8::from_slice(&a[4 * 8..4 * 8 + 8]));
        x2r = x0r - xr;
        x2i = x0i - xi;
        x0r += xr;
...
pub fn dct_benches(c: &mut Criterion) {
    ...
    group.bench_function("idct8x8", |b| b.iter(|| idct8x8(&mut arr_a)));

    let mut a = [0.0; 64];
    group.bench_function("idct8x8_simd", |b| b.iter(|| idct8x8_simd(&mut arr_b)));
