...
# Opinionated: also encodes into 6 subsampled JPGs (j444/j422/j420/444/422/420) for `just verify`
...
# --- encode subsampled variants (opinionated: always generate all 6) ---
echo "Encoding subsampled variants..."
declare -A MODES=(
	["j444"]="yuvj444p"
	["j422"]="yuvj422p"
	["j420"]="yuvj420p"
...
for suffix in "${!MODES[@]}"; do
	...
	output="$DIR/$BASE.${suffix}.input.jpg"
	echo "  $OUT -> $output ($pix_fmt)"
	ffmpeg -hide_banner -loglevel error -y -i "$OUT" -pix_fmt "$pix_fmt" "$output"
	ls -lh "$output" | awk '{print "    ", $5, $9}'
done
echo "All variants: $DIR/$BASE.{j444,j422,j420,444,422,420}.input.jpg"
